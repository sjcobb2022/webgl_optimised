import {ARRAY_BUFFER, ELEMENT_ARRAY_BUFFER, POINTS, TRIANGLES} from "./util/constants.mjs";
...
class Renderer {
    ...
    render(camera) {
        ...
        //scene.update()
        //if(scene.needsWorldUpdate === true){scene.update()}
        ...
        for(let s = 0; s<shaders.length;s++){
            ...
            const shader = shaders[s];

            //skips a bunch of expensive junk like binding hte shader
            if(shader.children.length < 1){
                console.warn(`Shader ${shader.uuid} is not in use and will be skipped`);
                continue;
            }
            ...
            for(let i=0;i<shader.children.length; i++){
                const object = shader.children[i];
                ...
                if (!(State.vertexBuffer === object.geom.buffers.mainBuffer)) {
                    ...
                    for (let key in object.geom.attributes) {
...

                        {
                            ...
                            // console.log(`gl.vertexAttribPointer(${shader.info.attr[key]}, ${numComponents}, gl.FLOAT, false, ${stride}, ${offset})`)
                ...
                if (object.geom.indexed === true) {

                    {
                        ...
                        const offset      = 0;
                        gl.drawElements(object.geom.drawType, vertexCount, type, offset);
                    }

                }
                else {
                    // console.log(object.geom.attributes['aPosition'].data.length);
                    gl.drawArrays(object.geom.drawType, 0, object.geom.attributes['aPosition'].data.length / object.geom.attributes['aPosition'].size);
                }
    ...
    registerShader(shader){
        ...
        if (shader.isShader !== true) {
            throw `Registered shader myst be of type shader`;
        } else {
